...
public class D01_ArrayList {
	/*
		...
		- 데이터를 가장 효율적으로 다루기 위해 어떻게 해야 하는가를 다루는 학문
		  ex) - 수 많은 데이터중 특정 데이터를 찾기 위해서는 어떻게 하는게 가장 효율적일까?
		      - 수 많은 데이터들을 순서대로 재배치하는데 가장 효율적인 방법은 무엇일까?
		
		- 세상에는 이미 수 많은 정답들이 알고리즘 형태로 존재하고 있다
		
	 	# Java Collections
	 	- 자료 구조들을 구현 해놓은 클래스들의 모임을 자바에서는 컬렉션이라고 부른다
	...
	public static void main(String[] args) {
		//기본적으로 ArrayList는 타입의 제한없이 모든 객체를 추가할 수 있다.
		ArrayList list = new ArrayList();
		
		
		// list.add(item) : 해당 리스트의 맨 뒤에 원하는 데이터를 추가할 수 있다.
		list.add("사과");
		list.add("오렌지");
		list.add("귤");
		list.add("배");
		...
		// add(index, data) : 원하는 index에 data를 삽입할수있다
		list.add(4, 999);
		...
		// list.get(Index) : 리스트의 해당 번째 요소를 꺼내 반환해 준다
		// 어떤 타입으로 꺼내야 할지 모르는 경우에는 Object타입으로 꺼낸다
		...
		System.out.println("삭제 성공값" +  deleted);
		System.out.println(scores);
		
		// list.remove(value) : 해당 값을 삭제한다
		// 일치하는 값이 여러개 인경우 앞에 하나만 지운다
		//  ( 성공여부를 반환한다)
		...
		System.out.println("30점있니? " + scores.contains(30));
		
		// list.set(index,value) : 해당 번째 값을 원하는 값으로 수정한다
		//						   (수정하면서 수정전 값을 반환해준다)
		int modified = scores.set(0, 9999);
		System.out.println("원래 있던값: " + modified);
		System.out.println(scores);
