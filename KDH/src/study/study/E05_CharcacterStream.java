...
public class E05_CharcacterStream {
	/*
	 	# inputStream/ OutStream
	 	- byte단위로 데이터를 입/출력 하는 클래스
	 	- 쓸때는 데이터를 byte형태로 분해해서 써야한다
	 	- 읽을 때는 읽은byte를 합쳐서 데이터를 만들어서 써야한다
	 	
	 	# Reader/Writer
	 	- char 단위로 데이터를 입/출력 하는 클래스
	 	- 쓸 때는 알아서 char를 byte로 분해해준다(직접 분해할 필요가 없다)
	 	- 읽을 때는 알아서 byte를 합쳐서 char형태로 우리에게 제공해준다
	 */
	public static void main(String[] args) {
		...
		try {
			...
			// 한 글자(char단위)씩 읽기, 더읽을것이 없으면 -1을 반환
//			int ch;
//			int i = 0;
//			while((ch = in.read()) != -1) {
//				//System.out.printf("%d번쨰 문자: %c\n", i++, ch);
//				System.out.print((char)ch);
//			}
			System.out.println("-------------------------------------");
			// 여러 글자씩 읽기. 기본적으로는 읽은 문자의 개수를 반환, 읽을게 없으면 -1 반환
			
			// 버퍼를 사용해 print() 메서드를 호출하는 휫수가 줄어들어 성능이 향상될수 있다
			char[] buff = new char[50];
			int len;
			while((len = in.read(buff)) != -1) {
				// 읽어드린 길이 만큼만 문자열로 만들어야 한다
