...
public class B02_Operator1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("(정수와 실수의 나누기) a / c: " + a / c);
		System.out.println("a % b: " + a % b);
		System.out.println("a % c: " + a % c);
		...
		// Math.round(x) : x를 소수 첫째 자리에서 반올림 해주는 함수
		System.out.println("round: " + Math.round(1.55));
		
		// Math.ceil(x) : x를 소수 첫째 자리에서 올림해주는 함수
		System.out.println("ceil: " + Math.ceil(1.11));
		
		//Math.floor(x) : x를 소수 첫째 자리에서 내림해주는 함수
		System.out.println("floor: " + Math.floor(1.99));
		
		// Math.pow(x, y) : x의 y제곱을 구하는 함수
		System.out.println("pow: " + Math.pow(2, 10));
		System.out.println("pow: " + Math.pow(2, 9));
		
		// Math.sqrt(x) : x의 제곱근을 구하는 함수
		System.out.println("sqrt: " + Math.sqrt(49));
		System.out.println("sqrt: " + Math.sqrt(48));
		System.out.println("sqrt: " + Math.sqrt(9));
		
		// Math.max(x, y) : 두 값중 더 큰값을 구하는 함수
		System.out.println("max: " + Math.max(-3, 99));
		
		// Math.min(x, y) : 두 값중 더 작은값을 구하는 함수
		System.out.println("min: " + Math.min(-3, 99));
		
		
		System.out.println("------문제-----");
		// 연습10.34567을 소수 3번째 자리에서 내림한 결과를 출력해보세요
		System.out.printf("%.2f\n", Math.floor((10.34567)*100)/100);
		...
		// (3) 아까 100을 곱했으니까 다시 100.0 으로 나눠서 원래대로 돌려준다
